...
\setbeamercolor{background canvas}{bg=bglight}
\begin{frame}[plain]
    % Fondo con patrón de grilla sutil
    \begin{tikzpicture}[remember picture, overlay]
        % Grilla
        \foreach \x in {0,0.5,...,16} {
            \draw[gridcolor, line width=0.2pt]
    ...
    % Layout: título a la izquierda
    \begin{columns}[T]
        \begin{column}{0.58\textwidth}
            % Título principal
            {\fontsize{26}{31}\selectfont\bfseries\color{mainblue}%
                CLASIFICACIÓN DE DAÑO EN\\
                AISLADORES\\
                SÍSMICOS MEDIANTE MACHINE\\
                LEARNING\\
        ...
        \begin{column}{0.40\textwidth}
            % Espacio para ilustración del aislador
            % (incluir imagen si se tiene, o dejar el espacio)
            \vspace{0.5cm}
            \begin{center}
                \includegraphics[width=0.85\textwidth]{cap3_medicion_aislador_utec.jpeg}
            \end{center}
        \end{column}
    \end{columns}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=south east, xshift=-1.5cm, yshift=0.4cm]
