...
\begin{frame}{Eficiencia Computacional}
    ...
    \begin{columns}[T]
        \begin{column}{0.32\textwidth}
            \begin{block}{FFT + PCA + RF}
                24s entrenamiento\\
                116~ms inferencia
            \end{block}
        \end{column}
        \begin{column}{0.32\textwidth}
            \begin{block}{FFT + AE + CNN}
                27~min entrenamiento\\
                13~ms inferencia (9x más rápido)
            \end{block}
        \end{column}
        \begin{column}{0.32\textwidth}
            \begin{block}{Trade-off}
                RF práctico para reentrenar; AE+CNN más ágil en producción
            \end{block}
        \end{column}
    \end{columns}
    \vfill
    \centering
    \includegraphics[width=0.92\textwidth]{cap4_tiempos_training_inferencia.png}
\end{frame}
